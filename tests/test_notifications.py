...
class TestNotificationManager:
    ...
    @pytest.mark.asyncio
    async def test_send_notifications_empty_list(
        self, mock_notification_manager, test_site_config
    ):
        """Test sending notifications with empty watch list."""
        result = await mock_notification_manager.send_notifications(
            [], test_site_config
        )
    ...
    async def test_send_notifications_disabled(
        self, mock_aiohttp_session, mock_logger, sample_watch_list, test_site_config
    ):
        ...
        with patch("notifications.APP_CONFIG") as mock_config:
            ...
            result = await manager.send_notifications(
                sample_watch_list, test_site_config
            )
    ...
    @pytest.mark.asyncio
    async def test_send_notifications_no_webhook_url(
        self, mock_aiohttp_session, mock_logger, sample_watch_list
    ):
    ...
    @pytest.mark.asyncio
    async def test_send_notifications_with_failures(
        self, mock_aiohttp_session, mock_logger, sample_watch_list, test_site_config
    ):
    ...
    @pytest.mark.asyncio
    async def test_send_notification_adds_muv_button(
        self,
        ...
        """Test MUV action button is attached when action storage is enabled."""
        ...
        try:
            ...
            assert call_args.kwargs["params"] == {"with_components": "true"}
            assert payload["components"][0]["components"][0]["label"] == "Send to MUV"
    ...
    @pytest.mark.asyncio
    async def test_send_single_notification_permanent_failure(
        self, mock_aiohttp_session, mock_logger
    ):
    ...
    @pytest.mark.asyncio
    async def test_send_single_notification_timeout(
        self, mock_aiohttp_session, mock_logger
    ):
        ...
        mock_logger.error.assert_called_with(
            "Timeout sending notification to Test Site"
        )

    @pytest.mark.asyncio
    async def test_send_single_notification_network_error(
        self, mock_aiohttp_session, mock_logger
    ):
    ...
    @pytest.mark.asyncio
    async def test_send_single_notification_unexpected_error(
        self, mock_aiohttp_session, mock_logger
    ):
    ...
    async def test_notification_rate_limiting(
        self, mock_aiohttp_session, mock_logger, test_site_config
    ):
    ...
    @pytest.mark.asyncio
    async def test_notification_embed_generation(
        self, mock_aiohttp_session, mock_logger, test_site_config
    ):
        ...
        with patch.dict(
            ...
            watch.to_discord_embed = Mock(
                return_value=original_to_embed(test_site_config.color)
            )
