...
class TestTropicalWatchScraper:
    ...
    def test_initialization(
        self, tropicalwatch_config, mock_aiohttp_session, mock_logger
    ):
        """Test scraper initialization."""
        scraper = TropicalWatchScraper(
            tropicalwatch_config, mock_aiohttp_session, mock_logger
        )
    ...
    @pytest.mark.asyncio
    async def test_extract_watches_success(
        ...
        assert (
            rolex_sub.url
            == "https://tropicalwatch.com/watches/rolex-submariner-date-116610ln"
        )
        assert rolex_sub.price == Decimal("10200")  # USD price parsed
        assert rolex_sub.currency == "USD"
        assert (
            rolex_sub.image_url
            == "https://tropicalwatch.com/images/rolex-submariner-116610ln.jpg"
        )
    ...
    @pytest.mark.asyncio
    async def test_extract_watches_empty_page(
        self, tropicalwatch_scraper, tropicalwatch_empty_html
    ):
    ...
    @pytest.mark.asyncio
    async def test_extract_watches_malformed_elements(
        self, tropicalwatch_scraper, tropicalwatch_malformed_html
    ):
    ...
    @pytest.mark.asyncio
    async def test_extract_watch_details_success(
        self, tropicalwatch_scraper, tropicalwatch_detail_html
    ):
    ...
    @pytest.mark.asyncio
    async def test_extract_watch_details_prefers_title_brand_when_table_is_wrong(
        self, tropicalwatch_scraper
    ):
        """Tropical Watch can publish stale table brand data on non-Rolex pages."""
        watch = WatchData(
        ...
        <html>
        <body>
            <h1 class="watch-main-title">1965 Omega Speedmaster "Ed White" 105.003 Tropical Dial</h1>
            <div class="watch-main-details-content">
                <table class="watch-main-details-table">
                    <tr><th>Year</th><td>1965</td></tr>
                    <tr><th>Brand</th><td>Rolex</td></tr>
                    <tr><th>Model</th><td>Speedmaster</td></tr>
                    <tr><th>Reference</th><td>105.003</td></tr>
                </table>
            </div>
        </body>
        </html>
        ...
        assert watch.model == "Speedmaster"
        assert watch.reference == "105.003"

    @pytest.mark.asyncio
    async def test_extract_watch_details_minimal_data(
        self, tropicalwatch_scraper, tropicalwatch_minimal_detail_html
    ):
    ...
    def test_model_extraction_from_title(self, tropicalwatch_scraper):
        ...
        temp_model_str = re.sub(
            rf"^{re.escape(watch.brand)}\s*", "", watch.title, flags=re.IGNORECASE
        ).strip()
        ...
        model_words = [
            word
            for word in temp_model_str.split()
    ...
    def test_reference_extraction_from_title(self, tropicalwatch_scraper):
        ...
        for title, expected_ref in test_cases:
            ...
            ref_match_title = re.search(
                r"\b([A-Z0-9]{3,}(?:[./\-\s]?[A-Z0-9]+)*)\b",
                temp_ref_search_str.strip(),
            )
            reference = None
            ...
            ), f"Reference extraction failed for: {title}"
    ...
    def test_case_material_extraction_from_title(self, tropicalwatch_scraper):
        ...
        for title, expected_material in test_cases:
            ...
            if "18k wg" in title_l or "white gold" in title_l:
                ...
                "18k pg" in title_l or "pink gold" in title_l or "rose gold" in title_l
            ):
            ...
            assert (
                case_material == expected_material
            ), f"Case material extraction failed for: {title}"

    @pytest.mark.asyncio
    async def test_box_papers_parsing(self, tropicalwatch_scraper):
        ...
        for description, expected_papers, expected_box in test_cases:
            ...
            with patch("scrapers.tropicalwatch.parse_box_papers") as mock_parse:
                ...
                assert (
                    watch.has_papers == expected_papers
                ), f"Papers detection failed for: {description}"
                assert (
                    watch.has_box == expected_box
                ), f"Box detection failed for: {description}"
    ...
    def test_diameter_extraction_from_description(self, tropicalwatch_scraper):
        ...
        for description, expected_diameter in test_cases:
            ...
            dia_match = re.search(
                r"(\d{2}(?:\.\d+)?)\s*mm", desc_text_for_dia, re.IGNORECASE
            )
            diameter = None
            if dia_match:
            ...
            ), f"Diameter extraction failed for: {description}"
    ...
    def test_parse_details_from_table_th_td(self, tropicalwatch_scraper):
        ...
        details = tropicalwatch_scraper._parse_details_from_table_th_td(
            table, headers_map
        )
    ...
    @pytest.mark.asyncio
    async def test_scrape_with_seen_watches(
        self, tropicalwatch_scraper, tropicalwatch_listing_html
    ):
        ...
        assert not any(
            watch.title == "Rolex Submariner Date 116610LN" for watch in watches
        )
