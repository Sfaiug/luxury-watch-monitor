...
class WatchOutScraper(BaseScraper):
    ...
    async def scrape(self) -> List[WatchData]:
        """Scrape Watch Out through Shopify's smaller collection JSON endpoint."""
        if "watch-out.shop" not in self.config.base_url:
            return await super().scrape()

        with PerformanceLogger(self.logger.logger, f"scraping {self.config.name}"):
            content = await fetch_page(
                self.session, self._products_json_url(), self.logger.logger
            )
            if not content:
                return await super().scrape()
            ...
            try:
                ...
                watches = self._extract_watches_from_json(data)
            except Exception as e:
                self.logger.warning(
                    f"Watch Out JSON parsing failed, falling back to HTML: {e}"
                )
                return await super().scrape()
    ...
    def _products_json_url(self) -> str:
        return urljoin(
            self.config.base_url,
            "/collections/neu-eingetroffen/products.json?limit=250",
        )

    def _extract_watches_from_json(self, data: dict) -> List[WatchData]:
        watches = []
        for product in data.get("products", []):
            watch = self._parse_product_json(product)
            if watch:
                watches.append(watch)
        return watches

    def _parse_product_json(self, product: dict) -> Optional[WatchData]:
        ...
        title = product.get("title") or "Unknown Watch"
        ...
        if raw_price not in (None, ""):
            try:
                ...
                price = parse_price(str(raw_price), "EUR")
        ...
        return WatchData(
            ...
            brand=brand,
            model=model,
            reference=variant.get("sku") or None,
            price=price,
            currency="EUR",
            image_url=image_url,
        )
    ...
    async def _extract_watches(self, soup: BeautifulSoup) -> List[WatchData]:
        ...
        try:
            script_tag_analytics = soup.find(
                "script", string=re.compile(r"window\.ShopifyAnalytics\.meta")
            )
            if script_tag_analytics and script_tag_analytics.string:
                match = re.search(
                    r"var meta = (\{.*?\})\s*;", script_tag_analytics.string, re.DOTALL
                )
                if match:
                    ...
                    if "products" in meta_data:
                        ...
                        self.logger.info(
                            f"Found {len(shopify_products_data)} items in Watch Out ShopifyAnalytics data."
                        )
                else:
                    self.logger.warning(
                        "Could not find 'var meta = {...}' in ShopifyAnalytics script for Watch Out."
                    )
            else:
                self.logger.warning(
                    "Could not find ShopifyAnalytics script tag for Watch Out."
                )
        except Exception as e:
        ...
        for idx, card_element in enumerate(product_card_elements):
            try:
                watch = self._parse_watch_element(
                    card_element, idx, shopify_products_data
                )
                if watch:
                    watches.append(watch)
            except Exception as e:
    ...
    def _parse_watch_element(
        self, card_element, idx: int, shopify_products_data: list
    ) -> Optional[WatchData]:
        ...
        card_title_text = (
            extract_text_from_element(card_title_tag) if card_title_tag else None
        )
        ...
        if idx < len(shopify_products_data):
            ...
            temp_shopify_title_variant = (
                temp_shopify_item.get("variants")[0].get("name")
                if temp_shopify_item.get("variants")
                else None
            )
            temp_shopify_title_product = temp_shopify_item.get(
                "untranslatedTitle", temp_shopify_item.get("title")
            )
            shopify_item_title = (
                temp_shopify_title_variant
                if temp_shopify_title_variant
                and temp_shopify_title_variant.lower() != "default title"
                else temp_shopify_title_product
            )

            analytics_prod_url_part = (
                temp_shopify_item.get("variants")[0].get("product", {}).get("url", "")
                if temp_shopify_item.get("variants")
            ...
            if handle and analytics_prod_url_part and handle in analytics_prod_url_part:
                ...
                card_title_text
                and shopify_item_title
                and card_title_text.lower() in shopify_item_title.lower()
            ):
                shopify_item = temp_shopify_item
        ...
        if shopify_item:
            watch_data["brand"] = (
                shopify_item.get("vendor") if shopify_item.get("vendor") else None
            )
            variant = (
                shopify_item.get("variants")[0] if shopify_item.get("variants") else {}
            )
            ...
            if variant_name and variant_name.lower() != "default title":
                ...
                title_candidate = shopify_item.get(
                    "untranslatedTitle", shopify_item.get("title")
                )
                if title_candidate:
        ...
        brand_tag_visual = card_element.select_one(
        ...
        ref_badge = card_element.select_one(
            ".product-card__badge-list span.badge--primary"
        )
    ...
    async def _extract_watch_details(self, watch: WatchData, soup: BeautifulSoup):
        ...
        main_desc_area = soup.select_one(
            "div.section-stack__intro div.metafield-rich_text_field div.prose"
        )
        main_desc_text = (
            extract_text_from_element(main_desc_area, separator=" ")
            if main_desc_area
            else ""
        )
        ...
        if accordion_data.get("herstellungsjahr"):
            watch.year = parse_year(
                accordion_data["herstellungsjahr"], watch.title or ""
            )
        ...
        if accordion_data.get("durchmesser"):
            ...
            dia_match = re.search(
                r"(\d{1,2}(?:[.,]\d{1,2})?)\s*mm", dia_text, re.IGNORECASE
            )
            if dia_match:
    ...
    def _parse_accordion_details_watch_out(self, accordion_box) -> dict:
        ...
        try:
            for collapsible in accordion_box.select("collapsible-element"):
                ...
                content_area = collapsible.select_one(
                    "div[id]"
