...
# Load environment before importing modules that initialize APP_CONFIG.
if Path(".env").exists():
    try:
...
def validate_environment() -> tuple[bool, list[str]]:
    ...
    for site_key, site_config in SITE_CONFIGS.items():
        ...
        if not webhook_url:
            required_webhooks.append(
                f"{site_config.webhook_env_var} (for {site_config.name})"
            )
...
Examples:
  ...
  python main_production.py --service start
    ...
    parser.add_argument(
        "--single", action="store_true", help="Run a single monitoring cycle and exit"
    )
    ...
    parser.add_argument(
        "--create-env", action="store_true", help="Create .env.example template file"
    )
    ...
    parser.add_argument(
        "--stats", type=int, metavar="DAYS", help="Show statistics for the last N days"
    )
    ...
    parser.add_argument("--log-file", help="Path to log file (creates rotating logs)")
    ...
    if args.reset_seen is not None:
        ...
        if not seen_items:
            ...
            sites_to_reset = (
                args.reset_seen if args.reset_seen else list(seen_items.keys())
            )
    ...
    monitor = WatchMonitor(log_level=args.log_level, log_file=log_file)
